...
export default function ProductListPageClient(){
  ...
  useEffect(()=>{ try{ window.scrollTo({ top:0, behavior:'smooth' }); }catch{ window.scrollTo(0,0); } },[page, pathname]);
  ...
  const filtered = useMemo(()=>{
    ...
    if (debounced.trim()){
      ...
      arr = arr.filter((p:any)=>{
        const hay = fold([p.title, p.slug, p.brand, p.model, p.category, p.subcategory].filter(Boolean).join(' '));
    ...
    if (ord === 'rand'){
      const seed = 1337;
      arr = arr.map((x:any,i:number)=>({x,k:Math.sin(i+seed)})).sort((a:any,b:any)=>a.k-b.k).map((o:any)=>o.x);
    } else {
    ...
    <div className="space-y-5">
      ...
      <div className="card-modern p-3">
        <div className="flex flex-wrap items-center gap-3">
          ...
          {(marca||cat||sub||debounced) && (
            <button className="chip" onClick={()=>{ router.push(pathname); setQ(''); }}>
              Limpar filtros
            </button>
          )}
        </div>
      ...
      {items.length === 0 ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
          {items.map((p:any)=> <ProductCard key={p.id||p.slug} product={p} />)}
        </div>
      )}
      ...
      {items.length>0 && totalPages>1 && (
        <nav className="flex justify-center items-center gap-2 select-none">
          ...
          {Array.from({length: totalPages}).map((_,i)=>(
            <button key={i} onClick={()=> setParam('page', String(i+1), false)} className={`chip ${i+1===curPage?'ring-2 ring-weg/60':''}`}>{i+1}</button>
          ))}
