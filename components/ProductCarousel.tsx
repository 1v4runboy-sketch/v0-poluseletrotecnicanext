...
export default function ProductCarousel({
  images, auto = true, interval = 4000, indicators = true, pauseOnHover = true, className = ''
}: {
  images: Img[]; auto?: boolean; interval?: number; indicators?: boolean; pauseOnHover?: boolean; className?: string;
}) {
  const [idx, setIdx] = useState(0);
  const [paused, setPaused] = useState(false);
  const startX = useRef<number | null>(null);

  const go = (n: number) => setIdx((i) => (n + images.length) % images.length);
  const next = () => go(idx + 1);
  const prev = () => go(idx - 1);
  ...
  return (
    <div className={`relative w-full h-full ${className}`}
      ...
      {images.length>1 && (
        <>
          <button className="carousel-arrow left-2" aria-label="Anterior" onClick={prev}><IconChevronLeft /></button>
          <button className="carousel-arrow right-2" aria-label="Próximo" onClick={next}><IconChevronRight /></button>
        </>
      )}
